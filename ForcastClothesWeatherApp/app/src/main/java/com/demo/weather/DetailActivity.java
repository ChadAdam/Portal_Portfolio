...
public class DetailActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, GestureDetector.OnGestureListener  {
    ...
    public void onLoadFinished(@NonNull Loader<Cursor> loader, Cursor o) {
        ...
        high_temp = Double.parseDouble(FormatUtils.formatTemp(this , max_far).substring(0, FormatUtils.formatTemp(this , max_far).length()-1));

        lower_temp = Double.parseDouble(FormatUtils.formatTemp(this , min_far).substring(0, FormatUtils.formatTemp(this , min_far).length()-1));;
        humid_percent= hum;
        wind = winSpeed;
    }
    ...
    private void onSwipeRight() {
       ...
       // double high_temp = Integer.getInteger(high_TV.getText().toString());
        //double lower_temp = Integer.getInteger(low_TV.getText().toString());
        //double humid_percent = Integer.getInteger(hum_TV.getText().toString());
        Context c = this;
        boolean isMetric = PreferenceUtils.isMetric(c);
        //Toast.makeText(c , "Metric "+ String.valueOf(isMetric) , Toast.LENGTH_LONG).show();
        Intent i = new Intent(c ,ClothesActivity.class);
        i.putExtra("Date", date_TV.getText());

        i.putExtra("humid",humid_percent/100);
        i.putExtra("wind", wind);
        if(!isMetric){
            i.putExtra("temp", (high_temp+lower_temp)/2);
        }
        else{
            double new_temp = ((high_temp+lower_temp)/2) * 1.8 + 32;
            i.putExtra("temp", new_temp);
        }
       // i.putExtra("temp", 65.00);
        startActivity(i);
